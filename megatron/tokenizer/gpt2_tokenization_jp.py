...
#  out=[20298, 20262, 5049, 20263, 211, 16050, 529, 19, 537, 16011, 5156, 7684, 15402, 20076, 6561, 20077, 
#  16011, 6408, 406, 17450, 20257, 214, 8413, 1007, 18073, 8184, 813, 406, 3, 20258], 
#  in.back= 「オタ」とも呼ばれているこのペラナカン（華人）の特製料理は、とてもお いしいスナック料理です。

#encode str2id: in=これは、ココナッツミルクやチリペースト、レモングラス、ガーリックと一緒に魚を砕き、
# それを、蒸して柔らかくしたバナナの葉に包んで炭火で軽く焼いた料理です。, 
#  out=[85, 17450, 20257, 2717, 3904, 15046, 1664, 17380, 18729, 4120, 407, 539, 20257, 3313, 553, 455, 
#  20257, 974, 2435, 17380, 17764, 269, 18737, 15227, 17804, 19791, 15707, 20257, 45, 17804, 20257, 19422, 
#  5, 15414, 1200, 1992, 15552, 4958, 15520, 16011, 18685, 18737, 15630, 239, 14797, 15541, 16898, 19341, 16617, 17661, 21, 406, 3, 20258], 
...
#  out=[20262, 5049, 5049, 20076, 20241, 20246, 20227, 20237, 20298, 20241, 20246, 20227, 20237, 20298, 
#  20077, 20263, 17450, 20257, 8301, 17119, 15790, 16898, 20262, 19960, 20263, 17804, 551, 67, 15335, 20258], 
#  in.back=「オタオタ（otak otak ）」は、マレー語で「脳」を意味します。

#encode str2id: in=この「オタオタ」という名前は、この料理の柔らかくトロリとした食感から由来しています。, 
#  out=[7, 20262, 5049, 5049, 20263, 12, 15498, 500, 17450, 20257, 7, 406, 16011, 15414, 1200, 16617, 1829, 
#  17402, 15, 15552, 4202, 0, 3666, 5, 138, 15335, 20258], 
...
#  out=[15227, 17804, 18888, 17, 5049, 5049, 16090, 20257, 15484, 18325, 873, 15398, 3, 16090, 20257, 4376, 
#  18729, 3401, 20257, 5990, 20257, 15227, 16011, 15014, 18, 17804, 15643, 21, 84, 18, 20257, 4, 3018, 1754, 1049, 15402, 17450, 4026, 3, 20258], 
...
class GPT2Tokenizer(object):
    ...
    def __init__(self, vocab_file, mecab_dict_path=None, emoji_file=None, 
        ...
        self.maxlen = max_len if max_len else np.max([len(w) for w in self.bpe]) #+ self.special_tokens]) # 13=self.maxlen, 13=<|endoftext|>, 12=<|emoji10|>, 11=<|emoji1|>
        if emoji_file:
    ...
    def clean_text(self, content):
        ...
        content = self.content_repatter4.sub("<DATE>" ,content) # TODO 当是TimeBERT的时候，不应该执行这个替换!
        content = self.content_repatter5.sub("<DATE>" ,content) # TODO not for TimeBert!
