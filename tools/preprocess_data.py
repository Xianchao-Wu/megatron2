    ...
    def initializer(self):
        ...
        if self.args.split_sentences: # 重要：第二个是负责句子切割的
            if not nltk_available:
                ...
                exit()
            if self.args.tokenizer_type == "BertWordPieceCaseJp" or self.args.tokenizer_type == "BertWordPieceJp" or self.args.tokenizer_type.startswith('GPT2BPETokenizerJp'):
...
def get_args():
    ...
    group.add_argument('--split-sentences', action='store_false', 
                       #action='store_true', #目前是默认split sentence of a document
    ...
    args.keep_empty = False # TODO, not used
...
def main():
    ...
    print(f"Vocab size: {tokenizer.vocab_size}") # 20573
    print(f"Output prefix: {args.output_prefix}") # my-gpt2-ja-debug
    output_bin_files = {}
    output_idx_files = {}
    builders = {}
